...
int DLT_ExtrinsicCameraParamEstimationWRansac_EPNP_mem(CvMat *X, CvMat *x, CvMat *K, CvMat *P, double ransacThreshold, int ransacMaxIter, vector<int> &vInlierOut);
...
int ExcludePointHighReprojectionError_mem_fast_Distortion_ObstacleDetection(vector<Feature> &vFeature, vector<CvMat *> &cP, vector<int> vUsedFrame, CvMat *X_tot, double omega, CvMat *K);
...
int VisibleIntersectionXOR3_mem_fast1(vector<Feature> &vFeature, int frame1, int frame2, vector<int> vFeatureID, vector<vector<double> > &cx1, vector<vector<double> > &cx2, vector<int> &visibleID);
...
int main ( int argc, char * argv[] )
{
	...
	vCamera[0].vvFeatureID.resize(max_nFrames);
	for (int iFeature = 0; iFeature < vFeature.size(); iFeature++)
	{
		for (int iFrame = 0; iFrame < vFeature[iFeature].vFrame.size(); iFrame++)
		{
			vCamera[0].vvFeatureID[vFeature[iFeature].vFrame[iFrame]].push_back(iFeature);
		}
	}	
	
	//for (int i = 0; i < camera.vvFeatureID.size(); i++)
	//	cout << vCamera[0].vvFeatureID[i].size() << " ";
	...
	while(vFrameOrder.size() != 0)
	{
		...
		for (int itemp = 0; itemp < vFrameOrder.size(); itemp++)
		{
			...
			int nVisible = VisibleIntersection23_Simple_fast(vFeature_temp_regi, cFrame1);
		...
		if (!DLT_ExtrinsicCameraParamEstimationWRansac_EPNP_mem(cX, cx, vCamera[(int)((double)cFrame/max_nFrames)].vK[iTakenFrame], P1, DLT_RANSAC_THRESHOLD, DLT_RANSAC_MAX_ITER, vInlierRANSAC))
		{
			//vFrameOrder.push_back(cFrame);
		...
		string save_nInlier = path + middleFolder + "inliers.txt";
		vnInlier.push_back(vInlierRANSAC.size());
		SaveVector(save_nInlier, vnInlier);
				
		CvMat *cx_r = cvCreateMat(vInlierRANSAC.size(), 2, CV_32FC1);
		CvMat *cX_r = cvCreateMat(vInlierRANSAC.size(), 3, CV_32FC1);
		
		for (int i = 0; i < vInlierRANSAC.size(); i++)
		{
			cvSetReal2D(cx_r, i, 0, cvGetReal2D(cx, vInlierRANSAC[i], 0));
			cvSetReal2D(cx_r, i, 1, cvGetReal2D(cx, vInlierRANSAC[i], 1));
			
			cvSetReal2D(cX_r, i, 0, cvGetReal2D(cX, vInlierRANSAC[i], 0));
			cvSetReal2D(cX_r, i, 1, cvGetReal2D(cX, vInlierRANSAC[i], 1));
			cvSetReal2D(cX_r, i, 2, cvGetReal2D(cX, vInlierRANSAC[i], 2));
		}
	
		//Pose_Ceres(P1, vCamera[(int)((double)cFrame/max_nFrames)].vK[iTakenFrame], omega, princ_x1, princ_y1, cX_r, cx_r);
		cvReleaseMat(&cX_r);
		cvReleaseMat(&cx_r);
		...
		for (int iUsedFrame = 0; iUsedFrame < vUsedFrame.size()-1; iUsedFrame++)
		{
			...
			//cout << vCamera[(int)((double)cFrame/max_nFrames)].vvFeatureID.size  << endl;
			if (!VisibleIntersectionXOR3_mem_fast1(vFeature, cFrame, vUsedFrame[iUsedFrame], vCamera[(int)((double)cFrame/max_nFrames)].vvFeatureID[iTakenFrame], cx1_vec, cx2_vec, visibleID1))
			{////
...
*/
			...
			//if (!ExcludePointHighReprojectionError_AddingFrame_mem_fast_Distortion(vFeature, cP, vUsedFrame, eVisibleID2, newX2, eVisibleID3, newX3_vec, omega, vCamera[0].K))
			if (!ExcludePointHighReprojectionError_AddingFrame_mem_fast_Distortion_ObstacleDetection(vFeature, cP, vUsedFrame, eVisibleID2, newX2, eVisibleID3, newX3_vec, omega, vCamera[0].K))
			{
				cvReleaseMat(&newX2);
				//cout << "6 No intersection from " << vUsedFrame[iUsedFrame] << "th frame" << endl;
				continue;
		...
		if ((int)cP.size() < 100)
		{
			...
			//int nStructure = ExcludePointHighReprojectionError_mem_fast_Distortion(vFeature, cP, vUsedFrame, X, omega, vCamera[0].K);
			int nStructure = ExcludePointHighReprojectionError_mem_fast_Distortion_ObstacleDetection(vFeature, cP, vUsedFrame, X, omega, vCamera[0].K);
			cout << "Number of cameras: " << cP.size() << endl;
			cout << "Number of structure: " << nStructure << endl;
			//SparseBundleAdjustment_MOTSTR_mem_fast(vFeature, vUsedFrame, cP, X, vCamera);
			//SparseBundleAdjustment_MOTSTR_mem_fast_Distortion(vFeature, vUsedFrame, cP, X, vCamera, omega);
