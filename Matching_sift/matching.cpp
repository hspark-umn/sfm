...
int main ( int argc, char * argv[] )
{
	...
	// int offset_min_x = atoi(argv[3]);
	// int offset_max_x = atoi(argv[4]);
	
	// int offset_min_y = atoi(argv[5]);
	// int offset_max_y = atoi(argv[6]);
	
	char temp[1000];
	string savepath = path + "reconstruction/";
	//sprintf(temp, savepath.c_str(), Begin);
	//savepath = temp;
	
	string savepath_m = savepath + "measurement/";
	string fileinfofile = path + "FileInfo.TXT";
	string filelist = path + "image/filelist.list";
	string filerange = path + "image_obd.txt";
	...
	// Camera cam;
	// cam.id = 0;
	// for (int iFile = 0; iFile < 10/*vFilename.size()*/; iFile++)
	// {		
	...
	//vector<bool> vRange_t, vRange;
	//LoadOBDData(filerange, vRange_t);
	
	//for (int i = Begin; i < End; i++)
	//	vRange.push_back(vRange_t[i]);
	//vRange = vRange_t;
	...
	for (int iFrame = 0; iFrame < vCamera[0].vTakenFrame.size(); iFrame++)
	{
		...
		vector<SIFT_Descriptor> vSift_desc, vSift_desc_t;
		...
		for (int isift = 0; isift < vSift_desc.size(); isift++)
		{
			//if (vSift_desc[isift].x<offset_min_x)
			//	continue;
			//if (vSift_desc[isift].x>offset_max_x)
			//	continue;
			//if (vSift_desc[isift].y<offset_min_y)
			//	continue;
			//if (vSift_desc[isift].y>offset_max_y)
			//	continue;
				
			//if (isift%2==0)
			//	continue;
			
			vSift_desc_t.push_back(vSift_desc[isift]);
		}
		vSift_desc = vSift_desc_t;
	...
	for (int iFC = 0; iFC < vFrameOrder.size(); iFC++)
	{
		...
		//if ((vFC[iFC1].vSift_desc.size() > 20)/* && (vRange[iFC1])*/)
		{
